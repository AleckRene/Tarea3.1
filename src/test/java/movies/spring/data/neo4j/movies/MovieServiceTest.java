...
class MovieServiceTest {
  ...
  private static final Neo4jContainer<?> neo4jContainer =
      new Neo4jContainer<>("neo4j:" + env("NEO4J_VERSION", "5")).withAdminPassword(PASSWORD);
  ...
  void searches_movies_by_title(@Autowired MovieService service) {
    ...
    assertThat(service.searchMoviesByTitle(title))
        .hasSize(2)
        .extracting(mr -> mr.movie().getTitle())
        .containsExactlyInAnyOrder("The Matrix Reloaded", "The Matrix Revolutions");
  }
  ...
  void fetches_d3_graph(@Autowired MovieService service) {
    ...
    assertThat(d3Graph)
        .isEqualTo(
            Map.of(
                "links", List.of(Map.of("source", 1, "target", 0)),
                "nodes",
                    List.of(
                        Map.of("label", "movie", "title", "The Matrix"),
                        Map.of("label", "actor", "title", "Keanu Reeves")) // /sin cambios
                ));
  }
