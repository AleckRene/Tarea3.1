...
public class MovieService {
  ...
  public List<MovieResultDto> searchMoviesByTitle(String title) {
    return this.movieRepository.findSearchResults(title).stream().map(MovieResultDto::new).toList();
  }

  /**
   * This is an example of when you might want to use the pure driver in case you have no need for
   * mapping at all, neither in the form of the way the {@link
   * org.springframework.data.neo4j.core.Neo4jClient} allows and not in form of entities.
   *
        ...
        movie
            ...
            .asList(
                (member) -> {
                  var result =
                      new CastMemberDto(
                          member.get("name").asString(), member.get("job").asString());
